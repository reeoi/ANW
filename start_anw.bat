...
cd /d "%~dp0"
echo ========================================
echo  ANW Auto Novel Writer 一键启动
echo ========================================
...
if "%ANW_REVIEW_HOST%"=="" set ANW_REVIEW_HOST=127.0.0.1
if "%ANW_REVIEW_PORT%"=="" set ANW_REVIEW_PORT=18000

echo [4.5/5] 释放端口 %ANW_REVIEW_PORT% ...
for /f "tokens=5" %%P in ('netstat -ano ^| findstr "LISTENING" ^| findstr /C:":%ANW_REVIEW_PORT% "') do (
...
if "%_KILLED%"=="1" (
  ...
  echo   端口 %ANW_REVIEW_PORT% 当前空闲。
)

echo [5/5] 启动 ANW Auto Novel Writer (托盘模式) ...
echo.
echo 访问地址: http://%ANW_REVIEW_HOST%:%ANW_REVIEW_PORT%
echo 任务栏右下角将出现 ANW 图标; 双击图标重新打开界面;
echo 右键图标可启停全自动 / 重启服务 / 退出。
echo.

if not exist ".venv\Scripts\pythonw.exe" goto :no_pythonw

start "" /B ".venv\Scripts\pythonw.exe" "tray_app.py" --host %ANW_REVIEW_HOST% --port %ANW_REVIEW_PORT%
...
set /a _ATTEMPT+=1
ping -n 2 127.0.0.1 >nul 2>&1
curl -sf -o nul http://%ANW_REVIEW_HOST%:%ANW_REVIEW_PORT%/api/health 2>nul && set "_OK=1"
goto :probe_loop
...
echo     应用日志:    logs\anw.log
echo.
echo 常见原因:
echo  - 端口被其它程序占用 [已尝试在第 4.5 步释放]
echo  - 依赖未安装完整 [用 start_anw.bat --reinstall 重装]
echo  - 防火墙拦截了本机 127.0.0.1 连接
pause
goto :end

:probe_ok
echo [OK] ANW 已在托盘后台启动；浏览器应自动打开。
echo 如果浏览器没打开，请手动访问: http://%ANW_REVIEW_HOST%:%ANW_REVIEW_PORT%
goto :end

:no_pythonw
echo [警告] 未找到 .venv\Scripts\pythonw.exe，回退到前台模式。
python -m review_queue.human_review --host %ANW_REVIEW_HOST% --port %ANW_REVIEW_PORT%
